...
class FaceDetector:
    ...
    def getFaceKeypoints(self, img, face_idx=0):        
        ...
        keypoints = []
        ih, iw, ic = img.shape
        for i in range(len(self.mpFaceDetection.FaceKeyPoint)):
            px = detection.location_data.relative_keypoints[self.mpFaceDetection.FaceKeyPoint(i).value].x
            py = detection.location_data.relative_keypoints[self.mpFaceDetection.FaceKeyPoint(i).value].y
            keypoint = {"name": self.mpFaceDetection.FaceKeyPoint(i).name, 
                        "keypoint" : (int(px*iw), int(py*ih))}
